...
export class FormLayoutsComponent implements OnInit{
  ...
  addPredict(){
    ...
    if(!this.checkForm(this.predict)){
      this.pred = this.checkForm(this.predict);
      console.log(this.checkForm(this.predict));
  ...
  btnClick = function () {
    if (this.checkForm(this.predict)=="accept this contract ") {
      this.router.navigateByUrl('/pages/miscellaneous/not-found');
    }
